...
const fastSpeed = 2
const slowSpeed = 0.2
export const GearTrain = forwardRef<Object3D>((props, robotRef) => {
  ...
  const radius = 1 * scale[0]
  const bodyLength = 10 * scale[0]
  const bodyHeight = 2 * scale[0] //2.2
  const bodyWidth = 1.2 * scale[0]

  const gearX = position.x || 1
  const gearY = position.y || 1
  const gearZ = position.z || 1
  ...
  return (
    <group onPointerDown={() => setMotorSpeed(slowSpeed)} onPointerUp={() => setMotorSpeed(fastSpeed)}>
      <ConstraintPart
        ...
        mass={0}
        type={'Static'}
        args={[bodyLength * scale[0], bodyHeight * scale[0], bodyWidth * scale[0]]}
        {...props}
        scale={[1,1,1]}
      >
        <Gear
        ...
        <Gear
          ...
          motorSpeed={0}
          // segments={32}
          position={[gearX - 2.25 * scale[0], gearY, gearZ]}
        />
        <Gear
          ...
          motorSpeed={0}
          // segments={32}
          position={[gearX - 5.5 * scale[0], gearY, gearZ]}
        />
        <Gear
          ...
          motorSpeed={0}
          // segments={10}
          position={[gearX + 1.95 * scale[0], gearY, gearZ]}
        />
      </ConstraintPart>
    </group>
  )
...
const Gear = forwardRef<Object3D, GearProps>(
  ({ motorSpeed = 0, radius = 0.7, position, leftSide, segments = 16, scale }, bodyRef) => {
    const parent = useContext(context)
